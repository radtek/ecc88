...
public class ReportServicesOld {
	...
	public static List<TendencyCheckDataBean> getRuntimeDatas(List<ReportDateError> rdList)
	{
		...
		if(rdlist.size()<1){
			TendencyCheckDataBean bean = new TendencyCheckDataBean();
			bean.setName(Labels.getLabel("ErrorFreeData"));
			bean.setErrorValue(Labels.getLabel("ErrorFreeData"));
			bean.setStart_when(Labels.getLabel("ErrorFreeData"));
			bean.setEnd_when(Labels.getLabel("ErrorFreeData"));
			rdlist.add(bean);
		}else{
			...
			if(sizeTotal>0){
				...
				bean.setName(Labels.getLabel("FailureStatistics"));
